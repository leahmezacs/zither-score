...
const noAuthModeDefaultConfig = {
    defaultAuthentication: {
        authenticationType: undefined,
    },
    additionalAuthenticationProviders: [],
};
const userPoolsDefaultConfig = {
    defaultAuthentication: {
        authenticationType: 'AMAZON_COGNITO_USER_POOLS',
    },
    additionalAuthenticationProviders: [],
};
const apiKeyDefaultConfig = {
    defaultAuthentication: {
        authenticationType: 'API_KEY',
    },
    additionalAuthenticationProviders: [],
};
const openIdDefaultConfig = {
    defaultAuthentication: {
        authenticationType: 'OPENID_CONNECT',
    },
    additionalAuthenticationProviders: [],
};
const iamDefaultConfig = {
    defaultAuthentication: {
        authenticationType: 'AWS_IAM',
    },
    additionalAuthenticationProviders: [],
};
const withAuthModes = (authConfig, authModes) => {
    const newAuthConfig = {
        defaultAuthentication: {
...
const getSchemaWithFieldAuth = (authDirective) => {
    return `
    type Post @model {
        id: ID!
        title: String!
...
const getSchemaWithTypeAndFieldAuth = (typeAuthDirective, fieldAuthDirective) => {
    return `
...
const getTransformer = (authConfig) => new graphql_transformer_core_1.default({
    transformers: [new graphql_dynamodb_transformer_1.DynamoDBModelTransformer(), new graphql_connection_transformer_1.ModelConnectionTransformer(), new ModelAuthTransformer_1.ModelAuthTransformer({ authConfig })],
});
const getObjectType = (doc, type) => {
...
describe('Validation tests', () => {
    ...
    test('AMAZON_COGNITO_USER_POOLS not configured for project', () => {
        validationTest(privateAuthDirective, apiKeyDefaultConfig, `@auth directive with 'userPools' provider found, but the project has no Cognito User \
Pools authentication provider configured.`);
    });
    test('API_KEY not configured for project', () => {
        validationTest(publicAuthDirective, userPoolsDefaultConfig, `@auth directive with 'apiKey' provider found, but the project has no API Key \
authentication provider configured.`);
    });
    test('AWS_IAM not configured for project', () => {
        validationTest(publicIAMAuthDirective, userPoolsDefaultConfig, `@auth directive with 'iam' provider found, but the project has no IAM \
authentication provider configured.`);
    });
    test('OPENID_CONNECT not configured for project', () => {
        validationTest(ownerOpenIdAuthDirective, userPoolsDefaultConfig, `@auth directive with 'oidc' provider found, but the project has no OPENID_CONNECT \
authentication provider configured.`);
    });
    test(`'group' cannot have provider`, () => {
        validationTest(groupsWithProviderAuthDirective, userPoolsDefaultConfig, `@auth directive with 'groups' strategy only supports 'userPools' provider, but found \
'iam' assigned`);
    });
    test(`'owner' has invalid IAM provider`, () => {
        validationTest(ownerWithIAMAuthDirective, userPoolsDefaultConfig, `@auth directive with 'owner' strategy only supports 'userPools' (default) and \
'oidc' providers, but found 'iam' assigned.`);
    });
    test(`'public' has invalid 'userPools' provider`, () => {
        validationTest(publicUserPoolsAuthDirective, userPoolsDefaultConfig, `@auth directive with 'public' strategy only supports 'apiKey' (default) and 'iam' providers, but \
found 'userPools' assigned.`);
    });
    test(`'private' has invalid 'apiKey' provider`, () => {
        validationTest(privateWithApiKeyAuthDirective, userPoolsDefaultConfig, `@auth directive with 'private' strategy only supports 'userPools' (default) and 'iam' providers, but \
found 'apiKey' assigned.`);
    });
});
describe('Type directive transformation tests', () => {
    const transformTest = (authDirective, authConfig, expectedDirectiveNames) => {
        const schema = getSchema(authDirective);
        const transformer = getTransformer(authConfig);
        const out = transformer.transform(schema);
        const schemaDoc = graphql_1.parse(out.schema);
    ...
    test(`'public' with IAM provider adds policy for Unauth role`, () => {
        ...
        const out = transformer.transform(schema);
        expect(out.rootStack.Resources[graphql_transformer_common_1.ResourceConstants.RESOURCES.UnauthRolePolicy]).toBeDefined();
    });
...
//# sourceMappingURL=MultiAuth.test.js.map