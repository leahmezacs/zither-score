        ...
        switch (_a.label) {
            ...
            case 2:
                ...
                (Array.isArray(selections[key]) &&
                    selections[key].includes('learn'))) return [3 /*break*/, 4];
...
var choicesFromMetadata = function (path, selection, isDir) {
    var templates = isDir
        ? fs.readdirSync(path).filter(function (f) { return fs.statSync(join(path, f)).isDirectory(); })
...
var getTriggerMetadata = function (path, selection) { return readJsonFile(path + "/" + selection + ".map.json"); };
...
var dependsOnBlock = function (context, triggerKeys, provider) {
    ...
    var dependsOnArray = context.updatingAuth && context.updatingAuth.dependsOn ? context.updatingAuth.dependsOn : [];
