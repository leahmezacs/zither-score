...
function run(context) {
    ...
    var selectProviders = providerPluginNames.length === 1 ? Promise.resolve({ selectedProviders: providerPluginNames }) : inquirer.prompt(providerSelection);
    return selectProviders.then(function (answers) {
        var configTasks = [];
        ...
        return sequential(configTasks).catch(function (err) {
            throw err;
        });
    });
}
