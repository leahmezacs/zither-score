...
function caseClassDeclarationForSelectionSet(generator, { caseClassName, parentType, fields, inlineFragments, fragmentSpreads, viewableAs }, beforeClosure, objectClosure) {
    ...
    if (!possibleTypes || possibleTypes.length == 1) {
        properties = fields.map(field => naming_1.propertyFromField(generator.context, field)).filter(field => field.propertyName != '__typename');
        language_1.caseClassDeclaration(generator, {
            caseClassName,
            params: properties.map(p => {
                return {
        ...
        generator.printNewlineIfNeeded();
        const properties = fields.map(field => naming_1.propertyFromField(generator.context, field)).filter(field => field.propertyName != '__typename');
        language_1.caseClassDeclaration(generator, {
            caseClassName,
            params: properties.map(p => {
                return {
