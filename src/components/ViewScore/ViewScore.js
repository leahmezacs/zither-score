...
class ViewScore extends Component {
  ...
  fetchRating = async event => {
    ...
    this.state.listComments.map(comment => (tempRate += comment.rating));
    this.setState({
    ...
    return (
      <div>
        <div className="jumbotron text-center viewScore_welcome">
          ...
          <h2>
            Last Updated: {new Date(this.state.score.updatedAt).toDateString()}
          </h2>
          <Box mb={3} borderColor="transparent">
            <div className="viewRating">
              ...
              <Box ml={1}>
                {this.state.avgRate
                  ? this.state.avgRate.toFixed(2) + " out of 5"
                  : "Not rated yet"}
              </Box>
            </div>
          </Box>
        ...
        <div id="main-content" className="score-scrollable">
          <g className="page-main">
            <div id="pdf-window">
              {this.generatePDF()}
              <iframe title={this.state.score.name} type="application/pdf">
